...
module.exports = function(req, res, next, use) {
  ...
  // parse nested query for ObjectIDs
  ...
  } else if(typeof query._id == 'object' && !(query._id instanceof ObjectID)) {
    recursiveParse(query._id);
  }
  
  if(query && (query.$or || query.$and || query.$nor)) {
    recursiveParse(query.$or);
  }
  ...
  function ready(err, db) {
    if(err) {
      ...
      db.collection(collection, function(err, collection) {
        ...
        // atomic delete
        if(req.method === 'DELETE' && (!req.query || !Object.keys(req.query).length)) {
  ...
  if(db.isConnected) {
    ...
    db.open(function(err, db) {
      ...
      if(!err && user) {
        db.authenticate(user, password, function(err, success) {
          if(success) {
            db.isConnected = true;
